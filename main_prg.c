...
void proc_echo(const char *input) {
    ...
    while (*rest) {
        if (*rest == '"') in_quotes = !in_quotes;
        else if (*rest == ' ' || *rest == '\t') {
            if (in_quotes) output[out_pos++] = *rest;
            else if (out_pos > 0 && output[out_pos - 1] != ' ') output[out_pos++] = ' ';
        } else output[out_pos++] = *rest;
        rest++;
    }
    
    if (!in_quotes && out_pos > 0 && output[out_pos - 1] == ' ') out_pos--;
    output[out_pos] = '\0';
    printf("%s\n", output);
}
...
int main() {
    ...
    while (1) {
        ...
        if (home_dir && strcmp(curr_dir, home_dir) == 0) {
            strcpy(display_dir, "~");
        } else {
            strcpy(display_dir, curr_dir);
        }
        
        printf("<%s@%s:%s> ", extract_usrname, extract_sysname, display_dir);
